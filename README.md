...
**Stats** — echoes seen, runs completed, picks, rerolls/banishes/freezes used, quality distribution, and the most picked and banned echoes. 

**Collection** — echoes relevant to your build with drop source, rolled status, tome ownership, base weight, and score. Filter by tome requirement, multi-rank echoes, class visibility, and drop source (open-world regions and raids).

**Policies** — echoes with non-default automation policies for quick review and editing.

**Affixes** — scanned affix sources for the build with apply preview and scan controls.

**Logbook** — the full session history embedded in the overview. A session starts at level 1 and ends when your character dies and resets. Every automation action is recorded with echo names, scores, target highlights, and remaining charges. Use **Export** to grab the selected run's log plus automation thresholds (peak and all five sliders) as they were at run start, for pasting into Discord or support threads. You can delete individual entries or clear all logs.
